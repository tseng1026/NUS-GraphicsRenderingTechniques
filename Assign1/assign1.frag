...
void drawBrickCube()
{
    if (gl_FrontFacing) {
		...
		// Transform perturbation vector to eye space.
		mat3 TBN = mat3(T, B, necNormal);
		vec3 ecPerturbedNormal = TBN * tanNormal;
		
		// Use eye-space perturbation vector as normal vector in lighting
		// computation using Phong Reflection Model.
		vec3 reflectVec = reflect(-lightVec, ecPerturbedNormal);
		float N_dot_L = max(dot(lightVec, ecPerturbedNormal), 0.0);
		float R_dot_V = max(dot(reflectVec, viewVec), 0.0);
		vec3 diffuse  = LightDiffuse.rgb  * texture(BrickDiffuseMap, uv).rgb * N_dot_L;
		vec3 ambient  = LightAmbient.rgb  * texture(BrickDiffuseMap, uv).rgb;
		vec3 specular = LightSpecular.rgb * pow(R_dot_V, MatlShininess);
...
void drawWoodenCube()
{
    if (gl_FrontFacing) {
		...
		// Determine whether fragment is in wood region.
		if (sqrDist > MirrorRadius * MirrorRadius) {
			// Perform Phong lighting computation using the wood texture
			// color as the ambient and diffuse material.
			vec3 reflectVec = normalize(reflect(-lightVec, ecNormal));
			float N_dot_L = max(dot(lightVec, ecNormal), 0.0);
			float R_dot_V = max(dot(reflectVec, viewVec), 0.0);
			vec3 diffuse  = LightDiffuse.rgb  * texture(WoodDiffuseMap, uv).rgb * N_dot_L;
			vec3 ambient  = LightAmbient.rgb  * texture(WoodDiffuseMap, uv).rgb;
			vec3 specular = LightSpecular.rgb * pow(R_dot_V, MatlShininess);
		...
		// Determine whether fragment is in mirror region.
		else {
			// Construct eye-space Tangent and Binormal vectors.
			vec3 T, B;
			compute_tangent_vectors(necNormal, ecPosition, p, T, B);
			
			// Construct tangent-space perturbation vector for a hemispherical bump.
			float PI = 3.1415926535;
			float side  = sqrt(sqrDist) / MirrorRadius;
			float ratio = cos(side * (PI / 2));
			vec3 tanPertubedNormal = normalize(vec3(p.x, p.y, ratio));

			// Transform perturbation vector to eye space.
			mat3 TBN = mat3(T, B, necNormal);
			vec3 ecPerturbedNormal = TBN * tanPertubedNormal;

			// Transform reflection vector to World Space.
			vec3 reflectVec = reflect(-viewVec, ecPerturbedNormal);
			
			// Use world-space reflection vector to access environment cubemap.
			vec3 color = texture(EnvMap, reflectVec).rgb;
