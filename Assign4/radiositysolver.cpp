...
// Need to set up the viewport, projection and view transfromation.
{
    ...
    **********************************************************/
    ...
    // The input parameter face is a number from 1 to 4, that indicates which face to set up for.
    float lookAt[3], diff[3];
    VecDiff(diff, shooterQuad->v[0], shooterQuad->v[1]);
    if (face == 1) { VecDiff(lookAt, shooterQuad->v[0], shooterQuad->v[1]);}
    else if (face == 2) { VecCrossProd(lookAt, shooterQuad->normal, diff);}
    else if (face == 3) { VecCrossProd(lookAt, diff, shooterQuad->normal);}
    else { VecDiff(lookAt, shooterQuad->v[1], shooterQuad->v[0]);}
    ...
    // and update the unshot power of their parent shooter quads.
{
    for (int i = 0; i < width * height; i++)
    {
        ...
        /**********************************************************
        ****************** WRITE YOUR CODE HERE ******************
        *************** R_j * B_i * F_ij * A_i/A_j ***************
        **********************************************************/
        // Use the color buffer (item buffer) to update the radiosities of the gatherer quads,
        // and update the unshot power of their parent shooter quads.
        for (int k = 0; k < 3; k++)
        {
            float power = m->gatherers[g]->surface->reflectivity[k] * shotPower[k] * dF / m->gatherers[g]->area;
            m->gatherers[g]->radiosity[k] += power;
            m->gatherers[g]->shooter->unshotPower[k] += power * m->gatherers[g]->area;
        }
    }
}
